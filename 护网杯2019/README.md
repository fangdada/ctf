...
&emsp;&emsp;<font size=2>然后我的思路是：**三字节key+12字节的输入，这两个东西我们全都未知；假设我们三字节的key已知，`rand()`每次返回的随机数也就已知，也就知道了上面`if(n&1)`的判断结果，知道了和输入flag的运算顺序；因此我们可以爆破key，然后将输入flag用符号代替求约束解，如果解存在就意味着这个key是有效的，我们就可以得知这个key应该就是正确的key，以此反推flag。**，然后就有了如下爆破脚本：</font></br>
